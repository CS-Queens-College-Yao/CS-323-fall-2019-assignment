...
public class HolidaySpecial {
    ...
    boolean canCookPreviousSteps(int initial, int current, int cook, int[][] signUpTable) {
        for (int checkStep = initial; checkStep <= current; checkStep++) {
            if (signUpTable[cook][checkStep] == 0) return false;
        }
        return true;
    }
