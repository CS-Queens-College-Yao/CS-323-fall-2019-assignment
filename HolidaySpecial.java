 ...
 * Does this compile or finish running within 5 seconds? N
 */
...
public class HolidaySpecial {
    ...
    public int[][] makeShifts(
        ...
        int mostStepsDone = 0;
        for (int i = 1; i <= numSteps; i++) { //starting with step 1 for each of our cooks
            int primaryChef;
            int brokeSteps = 0;
            for(int j = 1; i <= numCooks; j++) { //then going through each of the cooks
                int stepsDoneInterim = 0;
                for(int k = 1; k <= numSteps; k++) { //then finally we go through each step for every cook lol
                    if(signUpTable[j][k] == 1)
                        stepsDoneInterim++;
                    else
                    {
                        brokeSteps = k;
                ...
                if(stepsDoneInterim > mostStepsDone) {
                    mostStepsDone = stepsDoneInterim;
                    primaryChef = j;

                    for(int m = 1; m < brokeSteps; m++)
                    {
                        scheduleTable[j][m] = 1;
                    }
                }
