 ...
 *
 * You will only be graded on code you add to the makeShifts method. Do not mess
 * with the existing formatting and identation. You don't need to use the helper
 * methods, but if they come in handy setting up a custom test case, feel free
 * to use them.
 */
public class HolidaySpecial {
	 ...
	 * @param signUpTable An easy lookup tool, signUpTable[x][Y] = cook X signed up
	 *                    or did not sign up for step Y. Example: signUpTable[1][3]
	 *                    = 1 if cook 1 signed up for Step 3 signUpTable[1][3] = 0
	 *                    if cook 1 didn't sign up for Step 3
	 * @return scheduleTable: a table similar to the signUpTable where
	 *         scheduleTable[X][Y] = 1 means cook X is assigned to step Y in an
	 *         optimal schedule
	 */
	...
	public int[][] makeShifts(int numCooks, int numSteps, int[][] signUpTable) {
		...
		int cookOnShift = 0; // final cook on shift
		int longestShift = 0; // length of longest shift from that step
		for (int i = 1; i <= numSteps; i++) { // go from left->right for steps
			if (longestShift == 0) { // only goes in the for loop if the longestShift is 0 (over)
				for (int j = 1; j <= numCooks; j++) { // go from up->down for cooks
					int step = i;
					int streakShift = 0; // Initialize temp variables to copy step, and to keep track of cook's streak
					while (signUpTable[j][step] == 1) { // while the cook is available for the step
						streakShift++; // increments the counter (so at minimum its at 1)
						// System.out.println("Checking cook " + j + ", on step " + step + ", current
						// streakShift is " + streakShift);
						step++; // increments step to check for next step the next while loop
						if (step > numSteps) {
							break;
						} // if step after increment is greater than #steps, then it is out of bounds and
							// break out of while loop
						else if (signUpTable[j][step] == 0) {
							break;
						} // if next step is not in schedule break out of while loop
					} // end while loop
					if (streakShift > longestShift) {
						longestShift = streakShift;
						cookOnShift = j;
						// after while loop, if streakShift of that cook is the longestStreak for that
						// step
						// we replace the longestStreak with the new streak# to keep track of until all
						// chefs for that step is checked
						// and we also replace the cookOnShift with the longestShift cook
					}
				} // end cooks for loop
			}
			// System.out.println("longestShift: " + longestShift);
			// System.out.println("cookOnShift: " + cookOnShift);
			scheduleTable[cookOnShift][i] = 1; // assuming all steps guaranteed works, it will change the cook for that
												// step to 1
			longestShift--; // decrements the longest shift in preparation for the next loop
			if (longestShift == 0)
				cookOnShift = 0; // if the streak is 0 (over), it will reset the cookOnShift to 0
		} // end steps for loop
	...
	/**
	 * Makes the convenient lookup table based on the steps each cook says they can
	 * do
	 * 
	 ...
	 *         lookupTable[x][y] = 1 if cook x can do step y lookupTable[x][y] = 0
	 *         if cook x cannot do step y
	 */
	...
	/**
	 * Prints the optimal schedule by listing which steps each cook will do Example
	 * output is cook 1: 1, 3, 4
	 * 
	 * @param schedule The table of 0's and 1's of the optimal schedule, where
	 *                 schedule[x][y] means whether in the optimal schedule cook x
	 *                 is doing step y
	 */
	 ...
	 * delete any of your own test cases when you submit. The three recipe test
	...
	public static void main(String args[]) {
		...
		int[][] cookSignUps3 = { { 7, 10, 11 }, { 2, 3, 4, 5, 7 }, { 1, 5, 10 }, { 8, 9, 10 }, { 5, 6, 7, 8 },
				{ 1, 3, 4, 8 } };
