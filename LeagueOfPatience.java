...
public class LeagueOfPatience {
    ...
    public void myFastestPlay(int S, int T, Date startTime, int[][] durations) {
        int[][] times = new int[2][durations.length];
      //Making the times array 2D so we can reference the previous vertex. 0 for weight, 1 for previous vertex
        int numVertices = durations.length;
        for (int v = 0; v < numVertices; v++) {
            times[0][v] = Integer.MAX_VALUE;
        }
        ...
        for (int relax = 0; relax < numVertices - 1; relax++) {
            for (int startVertex = 0; startVertex < numVertices; startVertex++) {
                for (int endVertex = 0; endVertex < numVertices; endVertex++) {
                    if (durations[startVertex][endVertex] == 0) continue;
                    nextTime = getNextQuestTime(currentTime,startVertex,endVertex);
                    int timeUntilPlay = minutesBetween(currentTime, nextTime);
                    durations[startVertex][endVertex] = durations[startVertex][endVertex] + timeUntilPlay;
                    if (times[0][startVertex] != Integer.MAX_VALUE &&
                            times[0][startVertex] + durations[startVertex][endVertex] < times[0][endVertex]) {
                      times[0][endVertex] = times[0][startVertex] + durations[startVertex][endVertex];
                      times[1][endVertex] = startVertex;
                    }
                }
            }
