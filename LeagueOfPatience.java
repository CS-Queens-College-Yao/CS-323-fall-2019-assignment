    ...
    for(int count = 0 ; count < durations.length ; count ++ ) {
    	...
    	for(int v = 0 ; v < durations.length ; v++ ) 
    	{
    		if (!processed[v] && durations[u][v]!=0 && times[u] != Integer.MAX_VALUE ) { // check condition... 
            	...
            	int quest_time = times[u] + nextQuestTime;
    			
    			if(nextQuestTime >= quest_time)
    			{
    				wait_Time = nextQuestTime - quest_time;
    			}
    			else 
    			{
    				if((quest_time-nextQuestTime) == 0)
    				{
	        			wait_Time = 0;
	        		}
    				else
    				{		    	
    					wait_Time = durations[u][v] - (quest_time - nextQuestTime);     		
	        		}
	        	}
	        	
		    	if( times[u]+ durations[u][v] + wait_Time < times[v])
		    	{  
		    		times[v] = times[u] + durations[u][v] + wait_Time;
		    	}
	       }
