...
public class LeagueOfPatience {
	 ...
	 *
	 * @param startTime
	 *            the time you intend to start playing
	 * @param S
	 *            the s th location on the game map
	 * @param T
	 *            the t th location on the game map
	 * @param durations
	 *            durations[u][v] Table of how long game play between u and v
	 *            takes in minutes
	 */
	public void myFastestPlay(int S, int T, Date startTime, int[][] durations) {
		int[] times = new int[durations.length];
		// Your code along with comments here. Use the genericShortest function
		// for reference.
		// You want to do similar things as the generic shortest function,
		// except you want
		// to account for the time until the next quest time at each arrival at
		// a location.
		// Feel free to borrow code from any of the existing methods.
		// You will find the getNextQuestTime method and the minutesBetween
		// method helpful.
		// You can also make new helper methods.

		// THIS CODE WAS TAKEN AND MODIFIED FROM
		// https://www.geeksforgeeks.org/dijkstras-shortest-path-algorithm-greedy-algo-7/
		Boolean[] sPath = new Boolean[durations.length];
		Date curTime;
		...
		for (int j = 0; j < durations.length - 1; j++) {
			int nV = findNextToProcess(times, sPath);
			sPath[nV] = true;

			for (int k = 0; k <= T; k++) {

				curTime = getNextQuestTime(startTime, nV, k);
				if (durations[nV][k] != 0) {
					// System.out.println(curTime.getTime()); //gives value back in milliseconds
					// System.out.println(startTime.getTime());
					
					//fixed the minutesBetween() function to return a value in minutes
					//not seconds.
					durations[nV][k] += minutesBetween(startTime, curTime);

				}
				if (!sPath[k] && durations[nV][k] != 0 && times[nV] != Integer.MAX_VALUE
						&& times[nV] + durations[nV][k] < times[k]) {
					times[k] = times[nV] + durations[nV][k];
					startTime = curTime;
				}
			}
		}
		...
		// Extra Credit: Code below to print the suggested play path i.e. "2, 4,
		// 3, 5"
	}
	 ...
	 * 
	 * @param askingTime
	 *            the time at which the user needs to know when the next quest
	 *            is
	 * @param u
	 *            Look up quest starting point
	 * @param v
	 *            Look up quest endpoint
	...
	public int minutesBetween(Date time1, Date time2) {
		return (int) (time2.getTime() - time1.getTime()) / 60000;
	}
	 ...
	 * @param processed
	 *            boolean array tells you which vertices have been fully
	 *            processed
	...
	public void genericShortest(int graph[][], int source) {
		...
		// processed[i] will true if vertex i's shortest time is already
		// finalized
		...
		for (int count = 0; count < numVertices - 1; count++) {
			...
			// Update time value of all the adjacent vertices of the picked
			// vertex.
			for (int v = 0; v < numVertices; v++) {
				// Update time[v] only if is not processed yet, there is an edge
				// from u to v,
	...
	public static void main(String[] args) {
		...
		// You can create a test case for your implemented method for extra
		// credit below
	}
}
