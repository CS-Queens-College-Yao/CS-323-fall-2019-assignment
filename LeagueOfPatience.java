...
public class LeagueOfPatience {
  ...
  public void myFastestPlay(
   ...
   // and total weight of path from source to v through u is smaller than current value of time[v]+time_cost

        if (!processed[v] && durations[u][v]!=0 && times[u] != Integer.MAX_VALUE && times[u]+durations[u][v] < times[v]) {

          times[v] = times[u] + durations[u][v]+minutesBetween(getstartTime(startTime),getNextQuestTime(startTime,u,v))/60;
   ...
   */
  public Date getstartTime(Date askingTime) {
...
//      System.out.println( t.getNextQuestTime(date, 0, 1));
//      System.out.println( t.minutesBetween(t.getNextQuestTime(date, 0, 5), t.getNextQuestTime(date,0,0)));
//      System.out.println(
//      t.minutesBetween(t.getstartTime(date),t.getNextQuestTime(date,0,1))
//      );
